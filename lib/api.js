...
const API_TIMEOUT_MS = getPositiveNumber(
  process.env.API_TIMEOUT_MS,
  10_000
);

const CACHE_TTL_MS = getPositiveNumber(
  process.env.VERSIONS_CACHE_TTL_MS,
  45_000
);
...
let versionsRequestPromise = null;
...
function parseVersionsPayload(payload) {
  ...
  function addVersion(version, value, index) {
    const url = normalizeFileUrl(extractUrl(value));
...
async function getVersions(options = {}) {
  ...
  if (!forceRefresh && versionsRequestPromise) {
    return versionsRequestPromise;
  }

  versionsRequestPromise = (async () => {
  ...
  try {
    return await versionsRequestPromise;
  } finally {
    versionsRequestPromise = null;
  }
}
